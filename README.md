...
这本书写于2007年，其中一些idiom在C++11中可能已经有了更好的替代方法，要是我能够想到的话也会在翻译中更新。
...
C++允许重载取地址运算符，因此取地址运算符的返回值可能不是对象的地址。这样的做法极具争议，但语言自身并没有禁止允许这样做。取址器手法是不依赖取地址运算符获得对象实际地址的一种方法。

在下面的例子中，由于取地址运算符是类私有的，main 函数会编译失败,即使operator &可以访问，但从它的返回类型double转换到指针没有任何意义。
...
空基类经常出现在C++代码中。C++需要空的类有非零大小以便能够标识对象的身份。比如：一个空类对象的数组必须有非零

的大小，因为每个对象都用数组的下标标识，每个下标必须要不一样。指针算数会失败如果 sizeof(EmptyClass) 的大小是

零。通常的实现这样的类sizeof的结果通常是1。
...
当一个空的类是其他类的数据成员时，它占据了比1个字节更多的空间。为了防止对象切割，编译器通常需要把数据对齐到

四字节边界。空基类对象所占的四个字节只是占位符，没有其他实际的用途。为了节约内存，避免存储空间的浪费是人们

一直追求的，而且通常小体积的对象更cpu缓存友好。

##### 5.0.4 解决方案和示例代码

C++对空类做了特别的优化在空类作为基类的时候。编译器空基类不占空间。比如：下边的例子，sizeof(AnInt) 的结果是

4 在32位目标体系结构下，sizeof(AnotherEmpty) 的结果是1字节尽管他们都继承了 EmptyClass。
